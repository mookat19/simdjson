...
public:
  ...
  really_inline bool with_space_terminated_copy(const F& f) {
    ...
    */
    ...
    bool result = f(reinterpret_cast<const uint8_t*>(copy), parser.len);
    free(copy);
    return result;
  }
  ...
  really_inline bool at_end(uint32_t n_structural_indexes) {
    return current_structural == &parser.structural_indexes[n_structural_indexes - 1];
  }
  really_inline bool at_beginning() {
    return current_structural == parser.structural_indexes.get() - 1;
  }
};
